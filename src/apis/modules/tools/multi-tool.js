...
export async function multiToolRun(opts) {
    ...
    const getTemperature = (args) => {
        if (!cities.includes(args.city))
            return 'Unknown city';
    ...
    if (!res1.ok)
        throw new Error(`multiToolRun error: ${res1.status}`);
    ...
    if (assistantMsg.tool_calls?.length) {
        for (const tool of assistantMsg.tool_calls) {
            ...
            if (fn) {
                ...
                messages.push({ role: 'tool', content: output.toString(), tool_call_id: tool.id });
            }
        }
        ...
        if (!res2.ok)
            throw new Error(`multiToolRun final error: ${res2.status}`);
