...
function resolveMode(sourceIndices, explicitTarget) {
...
async function expandKeywords(message, endpoint, model, signal) {
    ...
    try {
        ...
        if (!match)
            return [message];
        const arr = JSON.parse(match[0]).filter((t) => typeof t === 'string' && t.trim());
...
async function searchAll(esEndpoint, keywords, targetIndex, sources = ['_all']) {
    ...
    if (!res.ok)
        throw new Error(`searchAll: ${res.status} ${res.statusText}`);
    const data = await res.json();
    const hits = data.hits?.hits || [];
    const seen = new Set();
    const refs = [];
    for (const h of hits) {
        if (h._index === targetIndex)
            continue;
...
async function ensureVectorIndex(esEndpoint, index, dims) {
...
async function reindexRefs(esEndpoint, refs, targetIndex) {
    var _a;
    const byIndex = {};
    for (const r of refs) {
        if (r.index === targetIndex)
            continue;
        (byIndex[_a = r.index] || (byIndex[_a] = [])).push(r.id);
    }
    const stats = [];
...
async function embedValue(endpoint, model, text, signal) {
    const controller = new AbortController();
    if (signal)
        signal.addEventListener('abort', () => controller.abort());
...
async function writeVectorKey(esEndpoint, targetIndex, message, keywords, endpoint, embeddingModel, matchedCount, refs, keyNames, signal) {
...
async function enrichEmbeddings(esEndpoint, targetIndex, arrayFields, endpoint, embeddingModel, signal) {
    var _a, _b;
        ...
        for (const field of arrayFields) {
            ...
            for (const item of val) {
                if (typeof item === 'string' && item.trim())
                    tasks.push({ hitId: hit._id, field, item });
            }
        }
    }
    // Embed all values in parallel, capped at 4 concurrent Ollama calls
    const embeddingResults = await pooled(tasks.map(t => () => embedValue(endpoint, embeddingModel, t.item, signal).then(vec => ({ ...t, vec }))), 4);
    // Group results back by hit id → field → item
    const byHit = {};
    for (const { hitId, field, item, vec } of embeddingResults) {
        if (!vec)
            continue;
        (byHit[hitId] ?? (byHit[hitId] = {}));
        ((_a = byHit[hitId])[_b = `${field}_embedding`] ?? (_a[_b] = {}))[item] = vec;
    }
    const bulkLines = [];
 ...
 */
async function reindexFullCopy(esEndpoint, sourceIndex, targetIndex) {
    const res = await fetch(`${esEndpoint}/_reindex`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
 ...
 */
async function writeReferencePointers(esEndpoint, targetIndex, refs, embeddings) {
    if (!refs.length)
        return 0;
    const lines = [];
    for (const ref of refs) {
...
async function ensureRefIndex(esEndpoint, index, dims) {
    const check = await fetch(`${esEndpoint}/${index}`, { method: 'HEAD' });
    if (check.status !== 404)
        return;
    const body = {
...
export async function vectorPipeline(opts) {
    const { message, ollamaEndpoints, chatModel, embeddingModel, esEndpoint, targetIndex: explicitTarget, dims = 768, arrayFields = ['tags', 'expertise_areas'], keyNames = [], sourceIndices, signal, } = opts;
    ...
    if (mode === 'single') {
        ...
        const vectorKey = await writeVectorKey(esEndpoint, resolvedTarget, message, keywords, llmEndpoint, embeddingModel, copyStats.created + copyStats.updated, [], keyNames, signal);
        return {
            mode: 'single',
            keywords,
            sourceIndex,
    ...
    if (mode === 'multi') {
        ...
        await pooled(refs.map(ref => async () => {
            const text = keyNames
                .map(k => {
            ...
            if (vec)
                embeddings.set(`${ref.index}::${ref.id}`, vec);
        }), 4);
        ...
        const vectorKey = await writeVectorKey(esEndpoint, resolvedTarget, message, keywords, llmEndpoint, embeddingModel, refs.length, refs, keyNames, signal);
        return {
            mode: 'multi',
            keywords,
