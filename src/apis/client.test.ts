...
/**
    ...
    async function testC18PromptRouter(): Promise<TestResult> {
      ...
      try {
        ...
        const routedThinkingModel = client.modelRouter.resolve({ TaskType: 'thinking', Speed: 100, defaultModel: fallbackModel });
        ...
        const cfgResult = client.modelRouter.resolve({ TaskType: 'chat', Speed: 100, defaultModel: fallbackModel, requiredCaps: ['tools', 'thinking'] });
        emit(`  modelRouter(chat, Speed=100, requiredCaps=['tools','thinking']) → "${cfgResult}" (most capable)`);
    ...
    async function testC17PersonaSearchAndChat(): Promise<TestResult> {
      ...
      try {
        ...
        console.log(["persona",r1[0],r1[0].name,r1[0].icon]);

        const personas = await client.esEntities.Persona.list('-created_date', 1000);
            personas.forEach((p: any, i: number) => {
        });
        const randomIndex = Math.floor(Math.random() * personas.length);
        const randomPersona = personas[randomIndex];

        // Log the random persona's details
        console.log(`Random Persona:`, randomPersona);

        // Build persona system prompt (OpenAI-style system message)
        const systemPrompt = [
          randomPersona?.name ? `You are ${randomPersona.name}.` : '',
          randomPersona?.description?.trim() || '',
          randomPersona?.instructions?.trim() || '',
        ].filter(Boolean).join('\n');

        console.log("system prompt:"+systemPrompt);

        const fastModel = client.modelRouter.resolve({ TaskType: 'thinking', Speed: 100});
        console.log(fastModel);

        
        const improvedx = await lib.chatCompletion([ 
            { role: 'system', content: systemPrompt },
            { role: 'user', content: 'What is the biggest threat to coral reefs today?' },  ],
            { temperature: 0.8, model: fastModel});
    ...
    async function testC19Vision(): Promise<TestResult> {
      ...
      try {
        ...
        const SMELLY_URL = 'https://raw.githubusercontent.com/santenova/ollama-spaceship/refs/heads/main/images/ollama-spaceship.png';
        ...
        */
        ...
        const fallbackModel = getModel();
        const routedChat = client.modelRouter.resolve({ TaskType: 'chat', Speed: 100, defaultModel: fallbackModel });
        const routedThinking = client.modelRouter.resolve({ TaskType: 'thinking', Speed: 100, defaultModel: fallbackModel });
        const routedJson = client.modelRouter.resolve({ TaskType: 'json', Speed: 100, defaultModel: fallbackModel });
        ...
        const enhanced = await withAbort(client, 'promptRouter.enhance', (signal) =>
          client.promptRouter.enhance(rawPrompt, { TaskType: 'chat', Speed: 100, defaultModel: fallbackModel, signal })
        );
    ...
    async function testC20Solution(): Promise<TestResult> {
     ...
     // 3. Real HTTP-level integration — enhance() calls the real endpoint
        //    with the routed model and persona-aware system prompt, returning
        //    a real enhanced prompt from the LLM.
        const userMessage = `How can we cleanup the ocean?
        Context:
            [Describe what to do - e.g., ? etc.]

            Description:
            [What should be prioritised - e.g., ?]

            Platform:
            [Target platform - e.g., ?]

            Components:
            [Specific your Components]
...
`;
        const enhanced = await client.promptRouter.enhance(userMessage, {
            TaskType: 'thinking',
            persona: { name: 'Dr. Jacques Cousteau', description: 'famous oceanographer' }
          });
        ...
        const result = await client.integrations.Core.solution(enhanced);
        ...
        console.log(`  Result structure: manifest=${hasManifest} | personas=${result?.personas?.length || 0} | debate=${result?.debate?.length || 0}`);
    ...
    async function testC22ClientInfraWiring(): Promise<TestResult> {
      ...
      try {
        ...
        const routedChat = mr.resolve({ TaskType: 'chat', Speed: 100, defaultModel: fallbackModel });
        ...
        const routedJson = mr.resolve({ TaskType: 'json', Speed: 100, defaultModel: fallbackModel });
        ...
        const enhanced = await pr.enhance(raw, { TaskType: 'chat', Speed: 100, defaultModel: fallbackModel });
        if (typeof enhanced !== 'string' || !enhanced) throw new Error('promptRouter.enhance returned empty');
