...
const pad = (s, n, align = 'left') => align === 'left' ? String(s).padEnd(n) : String(s).padStart(n);
...
async function main() {
    ...
    console.log(color(C.bold + C.cyan, '├' + '─'.repeat(120) + '┤'));
    // Column headers
    const headerRow = `  ${pad('#', 3, 'right')}  ${pad('Model', 30)}  ${pad('Endpoint', 26)}  ${pad('Acc', 7, 'right')}  ${pad('✓', 5, 'right')}  ${pad('✗', 5, 'right')}  ${pad('FP', 4, 'right')}  ${pad('FN', 4, 'right')}  ${pad('Err', 4, 'right')}  ${pad('Tokens', 8, 'right')}  ${pad('ms/q', 6, 'right')}  ${pad('Perf', 6, 'right')}`;
