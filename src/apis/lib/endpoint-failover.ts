...
const UNHEALTHY_TTL_MS = 30_000; // retry unhealthy endpoints after 30s

function getHealth(endpoint: string): EndpointHealth {
...
function isAvailable(endpoint: string): boolean {
  const h = getHealth(endpoint);
...
export async function pingEndpoints(
  ollamaEndpoints: string[],
): Promise<Array<{ endpoint: string; healthy: boolean; latencyMs: number }>> {
  return Promise.all(
    ollamaEndpoints.filter(e => !!e).map(async (ep) => {
      ...
      try {
        ...
        if (healthy) markHealthy(endpoint); else markUnhealthy(endpoint);
...
export function getEndpointHealth(): Array<{ endpoint: string; healthy: boolean; failCount: number; lastCheckedAt: number }> {
