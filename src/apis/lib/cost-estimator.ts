...
function resolvePricing(model: string): { inputPerMillion: number; outputPerMillion: number } {
  // Exact match
  ...
  for (const [key, price] of Object.entries(MODEL_PRICING)) {
    if (modelLower.startsWith(key.toLowerCase())) return price;
  }
...
export function finaliseEstimate(
  estimate: CostEstimate,
  actualOutputTokens: number,
): CostEstimate {
  const outputCost = (actualOutputTokens / 1_000_000) * estimate.pricing.outputPerMillion;
  const inputCost  = (estimate.inputTokens / 1_000_000) * estimate.pricing.inputPerMillion;
  return {
    ...estimate,
...
export function getPricingTable(): typeof MODEL_PRICING {
