    ...
    try {
        ...
        if (res.ok) {
            ...
            const models = (data.models || [])
                .map((m) => ({ id: m.name || m.model, size: m.size || 0 }))
                .filter((m) => !!m.id);
            if (models.length) {
...
async function loadTestCases(index, limit) {
    const cfg = getEsConfig();
    try {
...
async function validateOne(ollamaEndpoints, model, tc, signal) {
    try {
        ...
        const { content, usage } = await chatCompletionWithUsage(ollamaEndpoints, model, [{ role: 'user', content: prompt }], { temperature: 0, max_tokens: 5, signal });
...
async function pooledMap(items, concurrency, fn, signal) {
...
export async function tripleValidation(ollamaEndpoints, defaultModel, opts = {}) {
    ...
    for (let mi = 0; mi < models.length; mi++) {
        ...
        const recordResult = (expected, predicted, raw, tc, usage, ms) => {
            if (opts.includePerCase)
                perCase.push({ utterance: tc.utterance, triple: tc.triple, expected, actual: predicted, passed: predicted !== null && predicted === expected, raw });
            if (predicted === null)
                errors++;
            else if (expected ? predicted : !predicted)
                correct++;
            else {
                wrong++;
                expected ? fn++ : fp++;
    ...
    if (!opts.signal?.aborted) {
        try {
            ...
            telemetry.emit(TelemetryEvents.TRIPLE_VALIDATION_COMPLETE, {
                error: 'Failed to store test results: ' + e?.message,
            });
        }
    }
    telemetry.emit(TelemetryEvents.TRIPLE_VALIDATION_COMPLETE, {
...
function resolveTestResultIndex() {
    const cfg = getEsConfig();
...
async function fetchModelSizeMb(ollamaEndpoints, model) {
    const ep = (ollamaEndpoints.find((e) => !!e) || 'http://127.0.0.1:11434').replace(/\/$/, '');
    try {
        const res = await fetch(`${ep}/api/models`);
        if (!res.ok)
            return 0;
        const data = await res.json();
        const found = (data.models || []).find((m) => (m.name || m.model) === model);
        if (found?.size)
            return Math.round((found.size / (1024 * 1024)) * 10) / 10;
        return 0;
    }
    catch {
        return 0;
 ...
 */
async function storeTestResults(scores, ollamaEndpoints, testCaseCount) {
    const cfg = getEsConfig();
