...
describe('createBatcher', () => {
    ...
    test('coalesces calls within the window into a single executor call', async () => {
        let execCount = 0;
        const executor = async (batch) => { execCount++; return batch.map(([n]) => n * 2); };
        const batched = createBatcher(executor, 20);
    ...
    test('rejects all pending calls when executor throws', async () => {
        const executor = async (_batch) => { throw new Error('exec-fail'); };
        const batched = createBatcher(executor, 10);
    ...
    test('does not flush an empty queue', async () => {
        let execCount = 0;
        const executor = async (b) => { execCount++; return b.map(() => 1); };
        createBatcher(executor, 5);
