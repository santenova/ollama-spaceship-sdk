...
async function runVariant(variant, ollamaEndpoints, defaultModel, signal) {
...
async function judgeResponse(prompt, response, metrics, ollamaEndpoints, defaultModel) {
    const schema = {
        type: 'object',
        properties: {
            scores: {
...
Each metric: score 1-10 (10 = best) and a one-sentence reasoning.`;
    try {
        const result = await chatCompletion(ollamaEndpoints, defaultModel, [{ role: 'user', content: judgePrompt }], { response_json_schema: schema });
...
async function persistABResult(result) {
...
export async function splitTest(variants, opts = {}, ollamaEndpoints, defaultModel) {
    ...
    const abResult = { variants, metrics, results, winner, created_date: new Date().toISOString() };
...
export async function getABTestHistory(limit = 20) {
    const cfg = getEsConfig();
    try {
