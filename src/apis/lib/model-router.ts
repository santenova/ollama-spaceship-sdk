...
function saveToStorageDirect(endpoint: string, map: Record<string, Record<string, number>>) {
  try {
 ...
 */
async function loadFromEs(endpoint: string): Promise<PersistedCache | null> {
  try {
    const es = getEsConfig();
    const res = await fetch(`${es.endpoint}/${ES_INDEX}/_doc/${docIdFor(endpoint)}`);
    if (!res.ok) return null;
    const data: any = await res.json();
    if (!data.found || !data._source?.map) return null;
    return {
      endpoint,
      map: data._source.map as Record<string, Record<string, number>>,
      ts: data._source.ts as number,
    };
  } catch { return null; }
}
 ...
 */
async function saveToEs(endpoint: string, map: Record<string, Record<string, number>>): Promise<void> {
  try {
    const es = getEsConfig();
...
function scheduleRefresh(ep: string) {
  ...
  if (Date.now() - _lastRefreshAttempt < REFRESH_COOLDOWN_MS) return;
...
function getCapabilityMap(): Record<string, Record<string, number>> {
  ...
  if (_capabilityCache && normalizeEp(_cacheEndpoint) === epN) return _capabilityCache;
...
function resolveCapability(
  ...
  for (const cap of caps) {
    if (map[cap] && Object.keys(map[cap]).length > 0) return cap;
  }
...
export const modelRouter = {
  ...
  get taskCapabilities(): Record<string, string[]> {
  ...
  resolveAll(taskType: TaskType, defaultModel: string): string[] {
    ...
    if (!bucket || !Object.keys(bucket).length) return [defaultModel].filter(Boolean);
