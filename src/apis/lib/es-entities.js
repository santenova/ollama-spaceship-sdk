 ...
 */
export function setIndexPrefix(prefix, discoveredIndices) {
    const p = (prefix || '').trim() || DEFAULT_INDEX_PREFIX;
    try {
        localStorage.setItem(INDEX_PREFIX_KEY, p);
    }
    catch { }
    ...
    if (Array.isArray(discoveredIndices)) {
        ...
        for (const idxName of discoveredIndices) {
            ...
            if (!ENTITY_INDEX_SUFFIXES[entityName]) {
                ENTITY_INDEX_SUFFIXES[entityName] = suffix;
            }
        }
    }
    try {
...
export function saveEsConfig(cfg) {
    try {
        localStorage.setItem(ES_CONFIG_KEY, JSON.stringify(cfg));
    }
    catch { }
}
...
const json = (res) => {
    if (!res.ok)
        throw new Error(`ES error ${res.status}: ${res.statusText}`);
    return res.json();
};
 ...
 */
export async function ensureEsIndex(endpoint, index, mappings) {
    try {
...
const ensureIndex = (endpoint, index) => ensureEsIndex(endpoint, index);
...
const parseSort = (sort) => {
    if (!sort)
        return [{ created_date: { order: 'desc' } }, { _doc: { order: 'asc' } }];
...
function createEsEntityHandler(entityName, getConfig) {
    ...
    return {
        ...
        async list(sort, limit, skip, fields) {
            ...
            if (sourceFilter(fields))
                body._source = sourceFilter(fields);
        ...
        async bulkCreate(dataArray) {
            ...
            items.forEach((item, i) => {
                if (item.index?._id && results[i])
                    results[i].id = item.index._id;
            });
            return results;
        },
