...
function summarizeRequestBody(rawBody: string | any): Record<string, any> {
  if (!rawBody) return {};
  try {
    const body = typeof rawBody === 'string' ? JSON.parse(rawBody) : rawBody;
    const summary: Record<string, any> = {};
    if (body.model) summary.model = body.model;
    if (body.stream !== undefined) summary.stream = body.stream;
    if (body.think !== undefined) summary.think = body.think;
    if (body.temperature !== undefined) summary.temperature = body.temperature;
    if (body.tools?.length) summary.toolCount = body.tools.length;
    if (body.response_format) summary.jsonSchema = true;
    if (body.input) {
    ...
    if (body.messages?.length) {
      ...
      summary.messages = body.messages.map((m: any) => {
        const entry: any = { role: m.role };
        if (typeof m.content === 'string') {
          entry.content = m.content.slice(0, 300);
        } else if (Array.isArray(m.content)) {
          entry.content = `[${m.content.length} parts: ${m.content.map((p: any) => p.type).join(', ')}]`;
        } else {
...
function summarizeResponseBody(data: any): Record<string, any> {
  if (!data || typeof data !== 'object') return {};
  const summary: Record<string, any> = {};
  const msg = data.choices?.[0]?.message;
  if (msg?.content) summary.content = String(msg.content).slice(0, 500);
  if (msg?.thinking) summary.thinking = String(msg.thinking).slice(0, 300);
  if (msg?.tool_calls?.length) {
    summary.toolCalls = msg.tool_calls.map((tc: any) => ({
  ...
  if (data.data?.[0]?.embedding) {
    ...
    summary.embeddingPreview = data.data[0].embedding.slice(0, 5).map((v: number) => Number(v.toFixed(4)));
  }
...
export async function trackedOllamaFetch(
  url: string,
  init: RequestInit,
  label = 'ollama',
): Promise<Response> {
