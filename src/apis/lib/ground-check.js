 ...
 */
async function fetchDocText(docId, esEndpoint) {
 ...
 */
export async function groundCheck(response, sourceDocIds, ollamaEndpoints, model, embeddingModel = 'nomic-embed-text') {
    const cfg = getEsConfig();
    // 1. Fetch source document texts
    const sourceTexts = await Promise.all(sourceDocIds.map(id => fetchDocText(id, cfg.endpoint)));
    const validSources = sourceTexts.filter(Boolean);
...
Task: Identify any specific claims in the response that are NOT supported by the sources above. Be precise — quote the problematic phrase. If the response is fully grounded, return an empty flags array. Also provide an overall confidence score (0 = fully hallucinated, 1 = fully grounded).`;
    ...
    try {
        const result = await chatCompletion(ollamaEndpoints, model, [{ role: 'user', content: judgePrompt }], { response_json_schema: schema, temperature: 0 });
